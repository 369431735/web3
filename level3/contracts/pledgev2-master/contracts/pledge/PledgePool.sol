...
contract PledgePool is ReentrancyGuard, SafeTransfer, multiSignatureClient {
    ...
    enum PoolState {
        MATCH,          // 匹配中 - 可以存款和借款
        EXECUTION,      // 执行中 - 合约在执行中
        FINISH,         // 已完成 - 合约正常结束
        LIQUIDATION,    // 已清算 - 触发清算条件后的状态
        UNDONE          // 未完成 - 异常终止
    }
    PoolState constant defaultChoice = PoolState.MATCH;

    bool public globalPaused = false;
    address public swapRouter;                          // DEX交易路由合约地址
    address payable public feeAddress;                  // 收费地址（收取手续费的地址）
    IBscPledgeOracle public oracle;                    // 预言机合约地址（获取价格）
    uint256 public lendFee;                            // 借出手续费率
    uint256 public borrowFee;                          // 借入手续费率
...
//_oracle: 预言机合约地址 用于获取代币价格信息 实现接口 IBscPledgeOracle 必须是非零地址
//_swapRouter: DEX交换路由合约地址 用于执行代币之间的交换操作 通常是像 PancakeSwap 这样的 DEX 路由合约 必须是非零地址
//_feeAddress: 手续费接收地址 用于接收系统中产生的各种手续费 类型为 payable，可以接收原生代币(BNB) 必须是非零地址
//_multiSignature: 多签合约地址 用于合约的多重签名管理 通过 multiSignatureClient 父合约的构造函数传入 必须是非零地址
    constructor(
        address _oracle,
        address _swapRouter,
